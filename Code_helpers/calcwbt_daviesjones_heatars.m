function [Twb,Teq,epott]=calcwbt_daviesjones(TemperatureC,Pressure,Humidity,HumidityMode,ConvergenceMode,ConvTolerance,MaxIterations)
...
%FOR LONG CALCULATIONS, USE PYTHON INSTEAD -- SEE SPEEDYWETBULB.IPYNB OR
%WETBULB_DJ08_SPEDUP.IPYNB FOR MORE INFORMATION
...
% MATLAB port by Robert Kopp, June 2016

%
% Bug fixes by Qinqin Kong, Alex Goodman, and Rob Warren, implemented here
% by Colin Raymond, September 2023
    ...
    if ~exist('MaxIterations','var');MaxIterations=5000;end
        %original default for maxiterations is 20000, but smaller numbers mean faster calculation
    if ~exist('HumidityMode','var');HumidityMode=0;end
    if ~exist('ConvTolerance','var');ConvTolerance=0.00001;end
    ...
    if HumidityMode==0
        ...
        mixr=(qin./(1-qin)); 
        vape=(Pressure.*mixr)./(0.622+mixr); %Pa
        vapemb=vape./100;
        relhum = vape./es_mb; % relative humidity (%)
        mixr=mixr.*1000; %g/kg
    elseif HumidityMode==1
        relhum=Humidity;  % relative humidity (%)
        qin = 1000.*rs .* relhum * 0.01; % specific humidity (g/kg)
        vapemb = es_mb .* relhum * 0.01; % vapor pressure (mb) 
        mixr=1000.*0.622.*vapemb./(pmb-vapemb); %g/kg
    end
    ...
    sub=find(X<=D);
    %if length(sub)>1;disp('171');disp(sub);end
    ...
    if ConvergenceMode
        ...
        iter = 0;
        while (length(subdo)>0)&&(iter<=MaxIterations)
            iter = iter + 1;
...
% Clausius-Clapeyron
    ...
    foftk = ((Cf./T_k).^lambd_a).*(1 - es_mb./p0ndplam).^(vkp.*lambd_a).* ...
            exp(-lambd_a.*goftk);
    %fdT = -lambd_a.*(1./T_k + vkp.*de_mbdT./pminuse + gdT); %error found
    %by Qinqin Kong, corrected version immediately below
